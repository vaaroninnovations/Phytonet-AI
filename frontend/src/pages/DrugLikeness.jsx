...
export default function DrugLikeness() {
  ...
  // Reaching Drug-Likeness implies Plant Database + Toxicity Prediction are done.
  useEffect(() => {
    markComplete("plant-database");
    markComplete("toxicity-prediction");
  }, [markComplete]);

  const onContinue = () => {
    markComplete("drug-likeness-screening");
      ...
      >
        ...
        <div className="mt-10 flex flex-col items-start justify-between gap-4 rounded-3xl border border-[#E7E7F3] bg-white p-6 md:flex-row md:items-center">
          <div>
            ...
            <p className="mt-1 text-sm text-[#64748B]">
              Mark screening complete to advance to Target Prediction. The
              screening engine (Lipinski / Veber / Ghose / Egan + SwissADME) is
              still being tuned — your selection is safely staged for its run.
            </p>
          </div>
          <button
            data-testid="continue-drug-likeness-screening"
            onClick={onContinue}
            disabled={count === 0}
            className="inline-flex items-center gap-2 rounded-full bg-[#5139ED] px-6 py-3 text-sm font-semibold text-white transition-transform duration-200 hover:-translate-y-0.5 hover:bg-[#4127c9] disabled:pointer-events-none disabled:opacity-50"
          >
            Mark complete &amp; continue to Target Prediction
